...
import Core (Event (..), Input (Input), Vec2 (Vec2))
import Data.IORef (IORef, atomicModifyIORef', newIORef)
import Foreign (Int32, Ptr, Word64, alloca, nullPtr, peek)
import Foreign.C.String (peekCString)
import GLFW.Input (mapKey, mapKeyState, mapModifiers, mapMouseButton, mapMouseButtonState)
import GLFW.Instance (GLFW)
import Graphics.UI.GLFW qualified as GLFW
import Vulkan.Window (WindowStrategy (..))
...
glfwStrategy :: GLFW -> WindowStrategy GlfwWindow
glfwStrategy glfw =
    seq glfw $
        WindowStrategy
