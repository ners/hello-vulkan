...
import Vulkan.Builder (Builder, get, modify, put, runBuilder)
import Vulkan.Exception (VulkanException (..), throwIO)
import Vulkan.Queue (Queue (Queue), QueueFamily, QueueIndex, hasCompute, hasGraphics, hasTransfer)
import "vulkan" Vulkan qualified as Vk
import "vulkan" Vulkan.CStruct.Extends qualified as Vk
import "vulkan" Vulkan.Core10.Device hiding (createDevice, destroyDevice, withDevice)
import "vulkan" Vulkan.Core10.Device qualified as VkDCI (DeviceCreateInfo (..))
import "vulkan" Vulkan.Core10.Device qualified as VkDQCI (DeviceQueueCreateInfo (..))
...
newtype DeviceBuilder a = DeviceBuilder {builder :: Builder (Int, DeviceConfig) a}
...
solve physicalDevice config = do
  ...
  where
    ...
    sizeOf :: QueueFamily -> QueueMapping -> Word32
    sizeOf fam mapping =
        ...
        let
            ...
            queueCreateInfos =
                familyQueues <&> \(family, qs) ->
                    Vk.SomeStruct $
                        Vk.zero
                            { VkDQCI.queueFamilyIndex = family
                            , VkDQCI.queuePriorities = V.fromList qs
                            }
            createInfo =
                Vk.zero
                    { VkDCI.next = (config.features12, ())
                    , VkDCI.queueCreateInfos = V.fromList queueCreateInfos
                    , VkDCI.enabledExtensionNames = V.fromList $ fmap BS.pack config.extensions
                    }
