...
import Core (Vec2 (Vec2), clamp)
import Data.Bits.Extra (testAllBits)
import Data.List (nub, sortOn)
import Data.Vector qualified as V
import Data.Word (Word32, Word64)
import Vulkan.Builder (Builder, execBuilder, modify)
import Vulkan.Exception (VulkanException (..), throwIO, throwVk)
import Vulkan.Queue (Queue (familyIndex), QueueFamily)
import Vulkan.Result (expect)
import "vulkan" Vulkan qualified as Vk
import "vulkan" Vulkan.Core10.Pipeline qualified as VkVP (Viewport (..))
import "vulkan" Vulkan.Extensions.VK_KHR_swapchain
import "vulkan" Vulkan.Extensions.VK_KHR_swapchain as VkKHRS (SwapchainCreateInfoKHR (..))
...
toViewport minDepth maxDepth swapchain =
    Vk.zero
        { VkVP.x = 0
        , VkVP.y = 0
        , VkVP.width = fromIntegral swapchain.extent.width
        , VkVP.height = fromIntegral swapchain.extent.height
        , VkVP.minDepth = minDepth
        , VkVP.maxDepth = maxDepth
        }
...
renewSwapchain swapchain = do
    ...
    let
        ...
        transform =
            if testAllBits supportedTransforms wantedTransform
                then wantedTransform
                else currentTransform

    if realExtent.width <= 0 || realExtent.height <= 0
        then
            pure $
                swapchain
            ...
            let
                swapCreateInfo =
                    Vk.zero
                        { VkKHRS.surface = surface
                        , VkKHRS.minImageCount = imageCount
                        , VkKHRS.imageFormat = imageFormat
                        , VkKHRS.imageColorSpace = surfaceFormat.colorSpace
                        , VkKHRS.imageExtent = realExtent
                        , VkKHRS.imageArrayLayers = 1
                        , VkKHRS.imageUsage = supportedUsage
                        , VkKHRS.imageSharingMode = sharingMode
                        , VkKHRS.queueFamilyIndices =
                            if sharingMode == Vk.SHARING_MODE_EXCLUSIVE
                                then V.empty
                                else V.fromList families
                        , VkKHRS.preTransform = transform
                        , VkKHRS.compositeAlpha = Vk.COMPOSITE_ALPHA_OPAQUE_BIT_KHR
                        , VkKHRS.presentMode = presentMode
                        , VkKHRS.clipped = True
                        , VkKHRS.oldSwapchain = swapchain.handle
                        }
            newHandle <-
