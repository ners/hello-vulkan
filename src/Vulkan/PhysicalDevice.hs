...
import Vulkan.Queue (QueueFamily, hasCompute, hasGraphics, hasTransfer)
import Vulkan.Result (expect)
import Vulkan.Version (version)
import "vulkan" Vulkan qualified as Vk
import "vulkan" Vulkan.Core10.DeviceInitialization hiding (createInstance, destroyInstance, withInstance)
...
getMaxSampleCount physicalDevice = do
    ...
    pure $
        if testAllBits sampleCountFlags Vk.SAMPLE_COUNT_64_BIT
            then Vk.SAMPLE_COUNT_64_BIT
            else
                if testAllBits sampleCountFlags Vk.SAMPLE_COUNT_32_BIT
                    then Vk.SAMPLE_COUNT_32_BIT
                    else
                        if testAllBits sampleCountFlags Vk.SAMPLE_COUNT_16_BIT
                            then Vk.SAMPLE_COUNT_16_BIT
                            else
                                if testAllBits sampleCountFlags Vk.SAMPLE_COUNT_8_BIT
                                    then Vk.SAMPLE_COUNT_8_BIT
                                    else
                                        if testAllBits sampleCountFlags Vk.SAMPLE_COUNT_4_BIT
                                            then Vk.SAMPLE_COUNT_4_BIT
                                            else
                                                if testAllBits sampleCountFlags Vk.SAMPLE_COUNT_2_BIT
