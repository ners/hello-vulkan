...
import "vulkan" Vulkan.Core10.Memory
import Prelude hiding (map)

data Allocation (t :: Type) (k :: Type) = Allocation
    { key :: k
...
data MemoryStrategy (a :: Type) (k :: Type) = MemoryStrategy
...
writeBytes dstPtr bytes =
    liftIO $
        Unsafe.unsafeUseAsCString bytes $
            \srcPtr ->
