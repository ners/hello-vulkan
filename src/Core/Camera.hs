...
setAspectRatio
    :: Vec2
    -- ^ Width and height that define the aspect ratio.
    -> Camera
    -- ^ Old camera.
    -> Camera
    -- ^ New camera.
...
orbit
    :: Float
    -- ^ Horizontal rotation angle delta in radians (angle between x and y axes).
    -> Float
    -- ^ Vertical rotation angle delta in radians (angle between z axis and x-y-plane).
    -> Camera
    -- ^ Old camera.
    -> Camera
    -- ^ New camera.
...
approach distance camera =
    if distance < target
        then camera{position = newPosition}
        else camera
  where
